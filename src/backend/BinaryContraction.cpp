...
int64_t einsum_ir::backend::BinaryContraction::filter_dim_ids( int64_t         i_num_dims_tensor,
  ...
  for( int64_t l_di = 0; l_di < i_num_dims_tensor; l_di++ ) {
    if( i_dim_types_tensor[l_di] == i_dim_type_filter ) { //daniel: wenn dim typ nach dem man filtert gleich der dim typ des tensors an stell ist
...
einsum_ir::err_t einsum_ir::backend::BinaryContraction::compile_base() {
  ...
  for( int64_t l_c = 0; l_c < m_num_dims_c; l_c++ ) { //daniel: die dim sizes des dim typen C werden gepseichert
  ...
  if( m_loop_ids_ext != nullptr ){ //daniel:  [0] =0,[1] =1,[2] =2,[3] =3
